...
from utils.utils import get_optimizer, save_checkpoint, find_free_port


def parse_args():
    parser = argparse.ArgumentParser(description='Train DRNet')
    # general
...
def main():
    ...
    final_output_dir = create_logger(cfg, args.cfg, 'train')
    ...
    if cfg.DDP:
        ...
        main_worker(int(args.gpus), 1, dist_url, final_output_dir, args)
...
def main_worker(rank, world_size, dist_url, final_output_dir, args):
    ...
    # Data loading code
    train_loader = make_dataloader(cfg, is_train=True, distributed=cfg.DDP)
    ...
    trainer = Trainer(cfg, model, rank, final_output_dir)
